...
class YOLOFHead(AnchorHead):
    ...
    def loss(self, preds, gt_info,
             gt_bboxes_ignore=None,
             **kwargs):
        cls_scores = preds[0]
        bbox_preds = preds[1]
        device = cls_scores[0].device
        gt_bboxes = [torch.from_numpy(bboxes).to(device=device) for bboxes in gt_info['bboxes']]
        gt_labels = [torch.from_numpy(label).to(device=device, dtype=torch.int64) for label in gt_info['label_index']]
        ...
        anchor_list, valid_flag_list = self.get_anchors(
            featmap_sizes, gt_info, device=device)
        ...
        num_total_samples = cls_score.new_tensor(num_total_samples).clamp_(1.0).item()
