...
from..components.activations import HardSwish, HardSigmoid
...
class MobileNetV3(_MobileNetV3):

    def __init__(self,in_channels=3, out_levels=[1, 2,3,4,5], num_classes=None, mode="large", **kwargs):

        assert mode in ["large","small", "mobilenet_v3_small", "mobilenet_v3_large"]
        if mode == "large":
        ...
        if num_classes is None:
            super(MobileNetV3, self).__init__(inverted_residual_setting, last_channel, **kwargs)
            del self.classifier
            del self.avgpool
        else:
            super(MobileNetV3, self).__init__(inverted_residual_setting, last_channel, num_classes=num_classes, **kwargs)
    ...
    def get_stages(self):
        ...
        if self.mode == "mobilenet_v3_large":
            return [
                nn.Identity(),
                self.features[:2],
                self.features[2:4],
    ...
    def init_weights(self, pretrained=None):
        ...
        state_dict = load_state_dict_from_url(model_urls[self.mode])
