...
def load_pretrained(model, default_cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True, progress=False):
    ...
    if hf_hub_id and has_hf_hub(necessary=not pretrained_url):
        ...
        model_dir = os.path.join(os.getcwd(), "model_zoo","pth", "backbone", "timm")
        state_dict = load_state_dict_from_url(pretrained_url, progress=progress, map_location='cpu', model_dir=model_dir)
...
def remove_moudels(model, attributes):
    for attribut in attributes:
        if hasattr(model, attribut):
            delattr(model, attribut)