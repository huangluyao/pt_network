...
from networks.base.cnn.components import build_activation_layer
from networks.base.utils import is_list_of
from .fused_bias_leakrelu import fused_bias_leakyrelu, FusedBiasLeakyReLU
from .upfirdn2d import UpsampleUpFIRDn, Blur
from .equalize_module import EqualizedLRLinearModule, EqualizedLRConvModule, equalized_lr
...
class ModulatedPEConv2d(nn.Module):
    ...
    def __init__(self,
        ...
        # build blurry layer for upsampling
        if upsample and not self.deconkuv2conv:
            factor = 2
            p = (len(blur_kernel) - factor) - (kernel_size - 1)
            pad0 = (p + 1) // 2 + factor - 1
            pad1 = p // 2 + 1
...
class ModulatedPEStyleConv(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv = ModulatedPEConv2d(
            in_channels, out_channels, kernel_size, style_channels,
