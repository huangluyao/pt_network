...
class SSLWithGANHook(Hook):
    ...
    def before_train_iter(self, runner):
        ...
        # 对于 网络推理的结果做训练
        D_out_z = self._get_fake_D_result(runner)[0]
    ...
    def after_train_iter(self, runner):
        ...
        D_out, D_out_y_pred, pred_remain = self._get_fake_D_result(runner)
        # 寻找比较好的标签并参与训练
        pred_sel, labels_sel, count = find_good_maps(D_out, pred_remain, self.threshold_st)
        if count > 0 and runner.iter >0:
            loss_st = self.seg_loss(pred_sel, labels_sel)
        else:
            loss_st = 0.0
        D_gt_out, D_gt_out_pred = self._get_gt_D_result(runner)
    ...
    def _get_fake_D_result(self, runner):
        # 获取无标签的图像数据
        try:
