...
class MosaicDetection(Dataset):
    ...
    def __init__(self, dataset, img_size, is_gt_radio=0.7):
        super(MosaicDetection, self).__init__()
        self.dataset = dataset
        self.is_gt_radio = is_gt_radio
    ...
    def __getitem__(self, item):
        ...
        for i_mosaic, index in enumerate(indices):
            ...
            crop_result = RandomCrop(y2-y1, x2-x1, always_apply=True)(**labels)
            ...
            bboxes = bboxes[radio>=self.is_gt_radio]
            indexs = np.array(crop_result["label_index"])[radio>=self.is_gt_radio]
